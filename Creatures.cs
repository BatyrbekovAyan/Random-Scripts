...
public class Creatures : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        botRedList.Clear();
        activePortals = new();
        blueCreaturesTotal = (int)Mathf.Floor(40 + 20 * Mathf.Sqrt(Levels.level));
        ...
        subtructPower = 0;
        powerOfTen = 0;
        sizeRatio = PlaceholderRed.GetComponent<RectTransform>().rect.size.y / BotRed.transform.GetChild(0).GetComponent<RectTransform>().sizeDelta.y;
        blueCreaturesAppearInterval = 0.9f + Mathf.Sqrt(4f / Levels.level);
    ...
    private IEnumerator CreateCreature(int creatureIndex)
    {
        ...
        if (GameManager.stopCounting)
        {
            creature.transform.GetChild(2).GetChild(0).gameObject.SetActive(false);
        }
    ...
    private IEnumerator CreateBLue()
    {
        ...
        if (blueCreaturesToCreate[0] == null)
        {
            blueCreaturesToCreate.RemoveAt(0);
            blueCreationEnabled = true;

            yield break;
        }

        PlaceholderBlue.GetComponent<Image>().sprite = blueCreaturesToCreate[0].GetComponent<Creature>().sprite;
        ...
        botBlue.GetComponent<ParticleSystem>().GetComponent<Renderer>().material = blueCreaturesToCreate[0].GetComponent<Creature>().psMaterial;
        botBlue.GetComponent<BotBlue>().BotBlueImage.GetComponent<Image>().sprite = blueCreaturesToCreate[0].GetComponent<Creature>().sprite;
        ...
        botBlue.transform.GetChild(1).GetComponent<RectTransform>().localPosition = new Vector2(0, PlaceholderBlue.GetComponent<RectTransform>().rect.size.y / 2 + botBlue.transform.GetChild(1).GetComponent<RectTransform>().localPosition.y / 2 + 2 * Screen.height / Screen.width);
    ...
    {
        ShooterBots.transform.localPosition = new Vector2(0, 0.2f * StartUI.canvasHeight);
        shooterBotVerticalPos = Camera.main.ScreenToWorldPoint(new Vector3(0, 0, 0)) - Camera.main.ScreenToWorldPoint(new Vector3(0, 40, 0));

        shooterBotsNumber = 2 * ((Levels.level - 1) % sameLevelsNumber);
